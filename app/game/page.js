...
export default function GameLobby() {
  ...
  // 💡 데이터 초기 로딩 (최초 1회만 호출되어 덮어쓰기 버그 원천 차단)
  ...
  const handleOpenScrollBox = async () => {
    ...
    setTimeout(() => {
      ...
      else if (rand <= 50) setBlessedScrolls(prev => prev + 1);
      else setNormalScrolls(prev => prev + 1);
