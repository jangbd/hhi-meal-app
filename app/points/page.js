          ...
          <div className="space-y-5 my-auto animate-in fade-in duration-300 py-4">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
              ...
              <div className="flex items-center gap-3 mt-2 bg-red-50 p-4 rounded-xl border border-red-200 shadow-sm">
                <input type="checkbox" checked={regForm.hasDuplicate} onChange={(e) => setRegForm({...regForm, hasDuplicate: e.target.checked})} className="w-5 h-5 accent-red-600 rounded cursor-pointer" />
                <label className="text-[13px] font-black text-red-700 leading-snug">우리 부서에 저와 이름이 같은<br/>동명이인이 존재합니다.</label>
              </div>

              {regForm.hasDuplicate && (
                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                  <input placeholder="소속 마지막 상세 위치 (예: 선체1팀)" value={regForm.positionDetail} onChange={(e)=>setRegForm({...regForm, positionDetail: e.target.value})} className="w-full p-3.5 bg-white rounded-xl border border-indigo-200 font-bold outline-none focus:border-indigo-500" />
                </div>
              )}
